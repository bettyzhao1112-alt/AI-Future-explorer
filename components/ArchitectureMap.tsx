...
export const ArchitectureMap: React.FC<ArchitectureMapProps> = ({ activeLayer, onLayerSelect }) => {
  ...
  const getStroke = (layer: LayerType) => {
     return activeLayer === layer ? '#0ea5e9' : '#cbd5e1';
  }

  const getStrokeWidth = (layer: LayerType) => {
    return activeLayer === layer ? '4' : '2';
  }
    ...
    <div className="w-full h-full flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute inset-0 grid grid-cols-[repeat(20,minmax(0,1fr))] opacity-[0.03] pointer-events-none">
        {Array.from({ length: 400 }).map((_, i) => (
          <div key={i} className="border border-slate-900 aspect-square" />
        ))}
      </div>

      <svg viewBox="0 0 800 1000" className="w-full h-auto max-h-[85vh] drop-shadow-xl z-10 select-none">
        <defs>
        ...
        >
          <rect x="200" y="800" width="400" height="120" rx="10" fill="#ffffff" stroke={getStroke(LayerType.PHYSICAL)} strokeWidth={getStrokeWidth(LayerType.PHYSICAL)} className="shadow-lg" />
          <text x="400" y="840" textAnchor="middle" fill="#ef4444" fontSize="24" fontWeight="bold">物理层 (Physical)</text>
          <text x="400" y="870" textAnchor="middle" fill="#64748b" fontSize="16">数据中心 | GPU | 算力</text>
          {/* Server Icons */}
        ...
        >
          ...
          <rect x="250" y="680" width="300" height="80" rx="20" fill="#ffffff" stroke={getStroke(LayerType.NETWORK)} strokeWidth={getStrokeWidth(LayerType.NETWORK)} />
          <text x="400" y="715" textAnchor="middle" fill="#f59e0b" fontSize="24" fontWeight="bold">传输层 (Network)</text>
          <text x="400" y="740" textAnchor="middle" fill="#64748b" fontSize="16">光纤 | TCP/IP | 5G</text>
          {/* Packet Animation */}
        ...
        >
          <path d="M200 600 L600 600 L550 450 L250 450 Z" fill="#ffffff" stroke={getStroke(LayerType.VIRTUAL_DATA)} strokeWidth={getStrokeWidth(LayerType.VIRTUAL_DATA)} />
          <text x="400" y="520" textAnchor="middle" fill="#3b82f6" fontSize="24" fontWeight="bold">虚拟世界 (Virtual)</text>
          <text x="400" y="550" textAnchor="middle" fill="#64748b" fontSize="16">010101 | 大数据 | 向量</text>
        ...
        >
          <circle cx="400" cy="350" r="80" fill="#ffffff" stroke={getStroke(LayerType.INTELLIGENCE)} strokeWidth="4" />
          ...
          <text x="400" y="380" textAnchor="middle" fill="#8b5cf6" fontSize="14">Agent / LLM</text>
        ...
        >
          <rect x="100" y="100" width="200" height="100" rx="10" fill="#ffffff" stroke={getStroke(LayerType.APPLICATION)} strokeWidth={getStrokeWidth(LayerType.APPLICATION)} />
          <text x="200" y="140" textAnchor="middle" fill="#10b981" fontSize="20" fontWeight="bold">具身智能</text>
          <text x="200" y="170" textAnchor="middle" fill="#64748b" fontSize="14">机器人 | 自动驾驶</text>
          {/* Connection to Brain */}
        ...
        >
          <rect x="500" y="100" width="200" height="100" rx="10" fill="#ffffff" stroke={getStroke(LayerType.FUTURE)} strokeWidth={getStrokeWidth(LayerType.FUTURE)} />
          <text x="600" y="140" textAnchor="middle" fill="#ec4899" fontSize="20" fontWeight="bold">你的未来</text>
          <text x="600" y="170" textAnchor="middle" fill="#64748b" fontSize="14">学习方向 | 创造</text>
          {/* Connection to Brain */}
